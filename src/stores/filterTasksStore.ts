...
export const useFilterTasksStore = defineStore('filterTasks', {
    state: () => {
        ...
        const filters = {
            id: (todos: NewTask[]) =>
                todos.sort((a: NewTask, b: NewTask) => a.id - b.id),
            sortedDate: (todos: NewTask[]) =>
                todos.sort(
                    (a: NewTask, b: NewTask) =>
                        Number(new Date(a.date as string)) -
                        Number(new Date(b.date as string))
                ),
            active: (todos: NewTask[]) =>
                todos.filter((todo: NewTask) => !todo.done),
            completed: (todos: NewTask[]) =>
                todos.filter((todo: NewTask) => todo.done),
        };
        // вычисляемое состояние
        const filteredTodos = computed(() =>
            filters[visibility.value](useToDoStore().allTasks)
        );
